package view
{
	...
	public class MainWindow extends JWindow
	{
		...
		public function setPreviewShow(isShow:Boolean):void
		{
			MainWindow.toolBar.setVisible(!isShow);
			MainWindow.mapLayer.setVisible(!isShow);
			if (isShow)
			{
				getContentPane().append(MainWindow.previewWin, BorderLayout.CENTER);
				layerWin.hide();
				propertyWin.hide();
			}
			else
			{
				getContentPane().remove(MainWindow.previewWin);
				layerWin.show();
				propertyWin.show();
			}
		}
