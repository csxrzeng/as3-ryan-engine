package com.ryan.utils
{
	...
	public class LZMA
	{
		private static var flashVersion:int = -1;
		...
		private static function get needDecode():Boolean
		{
			if (flashVersion < 11)
			{
				return flashVersion == -1 ? (flashVersion = Capabilities.version.split(/[ ,]+/)[1]) < 11 : true;
			}
			return false;
		}
		...
		while (nowPos64 < outputSize)
		{
			...
			else
			{
				if (decodeBit(isRepDecoders, state) == 1)
				{
					...
					{
						var distance:int;
						if (decodeBit(isRepG1Decoders, state) == 0)
							distance = rep1;
						else
						{
							if (decodeBit(isRepG2Decoders, state) == 0)
								distance = rep2;
							else
							{
								distance = rep3;
